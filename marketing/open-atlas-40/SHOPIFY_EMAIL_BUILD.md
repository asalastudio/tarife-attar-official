...
| 3 | Text | centered, Arial 12px, color `#C9A227` | `THE ARCHIVE OPENS · 24 HOURS ONLY` |
| 4 | Heading | centered, Georgia *italic*, ~40px | `The Atlas, Open.` |
| 5 | Text | left, Georgia 16px | The three intro paragraphs ("Traveler —…") |
| 6 | Text | centered, on a white or bordered card if available; otherwise same section with generous spacing | `40%` huge (Georgia italic, ~70px) · `OFF EVERY WAYPOINT` · `EVERY TERRITORY · ALL FORMATS` · `ENDS TONIGHT · 11:59 PM PT` |
...
| 11 | Image-with-text or Text | section background `#0D0D0D`, text `#FAF9F6`, accent `#C9A227`; button style outlined gold, link `https://www.tarifeattar.com/gift` | Traveler Set band: `Ten waypoints, one satchel.` · `$95 → $57` · `CLAIM THE SET` |
...
**Subject/preheader:** subject A `the atlas opens today — and closes tonight`, preheader `Twenty-four hours. Twenty-eight waypoints. Forty percent off.` (variants in the copy deck).
...
Blocks: wordmark → gold divider → eyebrow `THE OPEN ATLAS · FINAL HOURS` → heading *The archive seals at midnight.* → two short paragraphs → offer line `40% OFF · EVERY WAYPOINT · UNTIL 11:59 PM PT` → Discount block → gold button `ENTER THE ATLAS` → text row `THE SEASON'S MOST CLAIMED: GRANADA · SAANA · MEDINA · THE TRAVELER SET — $57` → sign-off *Safe travels either way. — Jordan* → footer.

**E2–E4** are unused in the 24-hour format; `EMAIL_SEQUENCE.md` §9 keeps their swap map for the next opening.

## §4 · Audience segments (Shopify → Customers → Segments)

- **Every send:** the built-in **Email subscribers** segment (currently 8,434).
- **Suppress event purchasers on E5** — create segment `Open Atlas non-purchasers`:
  ```
  email_subscription_status = 'SUBSCRIBED' AND (last_order_date = NULL OR last_order_date < 2026-08-10)
  ```
  Re-save/verify it's refreshing before each send; use it as tonight's audience.
...
Skipped in the 24-hour format — E1 carries the rebrand story itself. The WARM-UP drafts stay in Omnisend for the next opening.

## §6 · Cost + test checklist

- Shopify Messaging bills per message beyond the monthly free tier. This event ≈ 2 sends × ~8.4k ≈ **~17k emails** — roughly $7 past the free 10k tier at historical pricing; check the app's pricing screen.
...
Nothing strategic. Timeline, offer, copy, guardrails, and projections all hold. Only the assembly swaps: Omnisend campaigns → Shopify Messaging campaigns; Omnisend segments (§4 there) → Shopify customer segments (§4 here); Omnisend popup/abandonment → keep in Omnisend if it stays connected to the store, or use Shopify's built-in abandoned-checkout automation (Marketing → Automations) with the §8 copy from the copy deck. With 8,434 subscribers at the 40% offer, the §6 projection model lands at roughly **$300 / $1,050 / $3,100** (conservative/base/stretch) email-attributed for the day, plus the on-site automatic discount catching everything else.
