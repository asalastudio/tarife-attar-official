...
- **Discount:** create the automatic discount + `VOYAGE40` code first (STRATEGY.md §2) — the editor's **Discount block** can then attach the real code, and Shopify appends it to links so it pre-applies at checkout.
...
- **Footer:** Shopify Messaging inserts the unsubscribe link and your store's physical address automatically — confirm the address in Settings → Store details. (This replaces the two `[[…]]` placeholders that exist in the HTML files for Omnisend.)
...
| 3 | Text | centered, Arial 12px, color `#C9A227` | `THE ARCHIVE OPENS · AUGUST 13–16` |
| 4 | Heading | centered, Georgia *italic*, ~40px | `The Atlas, Open.` |
| 5 | Text | left, Georgia 16px | The three intro paragraphs ("Traveler —…") |
| 6 | Text | centered, on a white or bordered card if available; otherwise same section with generous spacing | `40%` huge (Georgia italic, ~70px) · `OFF EVERY WAYPOINT` · `EVERY TERRITORY · ALL FORMATS` · `THROUGH SUNDAY, AUGUST 16 · 11:59 PM PT` |
| 7 | **Discount block** | attach `VOYAGE40` | Shopify renders the code + auto-apply link |
...
| 11 | Image-with-text or Text | section background `#0D0D0D`, text `#FAF9F6`, accent `#C9A227`; button style outlined gold, link `https://www.tarifeattar.com/gift` | Traveler Set band: `Five waypoints, one satchel.` · `$95 → $57` · `CLAIM THE SET` |
...
**Subject/preheader:** subject A `the atlas opens today`, preheader `Four days. Twenty-eight waypoints. Forty percent. Through Sunday night.` (variants in the copy deck).
...
Blocks: wordmark → gold divider → eyebrow `THE OPEN ATLAS · FINAL HOURS` → heading *The archive seals at midnight.* → two short paragraphs → offer line `40% OFF · EVERY WAYPOINT · UNTIL 11:59 PM PT` → Discount block → gold button `ENTER THE ATLAS` → text row `MOST CLAIMED THIS WEEK: GRANADA · SAANA · MEDINA · THE TRAVELER SET — $57` → sign-off *Safe travels either way. — Jordan* → footer.
...
- **Suppress event purchasers on E2–E5** — create segment `Open Atlas non-purchasers`:
  ```
  email_subscription_status = 'SUBSCRIBED' AND (last_order_date = NULL OR last_order_date < 2026-08-13)
  ```
...
## §5 · Warm-up sends (Tue/Wed) in Shopify Messaging

The WARM-UP drafts live in Omnisend and won't transfer. Recreate the two we use as simple text emails (5 min each — no design needed, personal-note style converts better anyway):
- **Tue:** WARM-UP 2 — subject `which one are you?` — body from the Omnisend draft (territory quiz invitation, link to `/quiz`). Plain text blocks only, from Jordan.
- **Wed:** WARM-UP 4 — subject `your favorite scent has a new name` — rebrand reveal + the tease paragraph from `EMAIL_SEQUENCE.md` §1.
...
Nothing strategic. Timeline, offer, copy, guardrails, and projections all hold. Only the assembly swaps: Omnisend campaigns → Shopify Messaging campaigns; Omnisend segments (§4 there) → Shopify customer segments (§4 here); Omnisend popup/abandonment → keep in Omnisend if it stays connected to the store, or use Shopify's built-in abandoned-checkout automation (Marketing → Automations) with the §8 copy from the copy deck. With 8,434 subscribers, the §6 projection model lands at roughly **$600 / $1,800 / $4,500** (conservative/base/stretch) email-attributed, plus the on-site automatic discount catching everything else.
