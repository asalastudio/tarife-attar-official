...
**Campaign:** The Open Atlas — a four-day opening of the archive. 40% off every Atlas waypoint.
**Window:** Thursday Aug 13 → Sunday Aug 16, 2026, 11:59 PM PT (warm-up begins Tue Aug 11)
...
| `emails/01-the-atlas-opens.html` | Production HTML for the announcement email (paste into Omnisend → "Code your own") |
...
- **The Traveler Set** (5-piece set, $95, 17 in stock) is the AOV weapon: $57 at 40% off.
...
1. Discounts → Create → **Automatic discount** → "THE OPEN ATLAS — 40%" → 40% off → applies to collections `ember`, `tidal`, `petal`, `terra`, `gift-sets` → active Aug 13 12:00 AM PT → Aug 16 11:59 PM PT.
2. Discounts → Create → **Code** `VOYAGE40`, identical rules and window. (Automatic and code won't stack; the code simply also works.)
...
Six days, seven sends. Story first, then the event. All times PT (brand timezone).

| Day | Send | To | Job |
|---|---|---|---|
| **Tue Aug 11, 8am** | WARM-UP 2 — "which one are you?" *(existing draft)* | Full list | First-ever send. Pure engagement (territory quiz), zero selling. Flushes hard bounces, starts sender reputation, generates open/click data that becomes our segments |
| **Wed Aug 12, 8am** | WARM-UP 4 — "your favorite scent has a new name" *(existing draft + one added tease line)* | Full list minus Tue hard bounces | The rebrand reveal. Add closing line: *"Tomorrow, to mark the completed Atlas, we're opening it — properly. Watch for the morning post."* |
| **Thu Aug 13, 8am** | **E1 — The Atlas, Open** (`emails/01-the-atlas-opens.html`) | Full list minus bounces/complaints | The announcement. 40%, the reason, the code, four featured waypoints, The Traveler Set, quiz for the undecided |
| **Fri Aug 14, 9am** | E2 — Four territories, one decision | Non-purchasers | Territory guide — helps the overwhelmed choose. Deep links to each territory |
| **Sat Aug 15, 9am** | E3 — The Traveler Set + what travelers are claiming | Non-purchasers, weighted to openers | Social proof (real 90-day bestsellers), the $57 set, 3ml minis as low-risk entry |
...
**Why this shape:** flash-sale revenue reliably concentrates ~40–50% in the final 24 hours — that's why Sunday gets two sends and the best creative. The two warm-ups are not a delay; they are what gets Thursday's announcement into the inbox instead of spam.

**Aggressive alternative (if you refuse to wait):** skip the warm-ups, fold the rebrand story into E1, launch Wednesday. You gain a day of selling and accept meaningfully higher spam/complaint risk on a cold list. Not recommended — but E1's copy already carries the rebrand story, so it works standalone if you choose this.
...
3. **Event Purchasers** — placed order in the last 7 days (requires the Shopify↔Omnisend store connection to be live — verify order events are flowing before Thursday; the brand shows `connected: true` but do a test order or check a recent real order appears on the contact timeline) — **used as an exclusion** on E2–E5
...
- **Cart (Satchel) abandonment:** enable Omnisend's abandoned-checkout automation before Thursday with the event-aware override copy provided (§7 of the copy deck) — 1-hour delay, one email. During a 4-day window, a 24-hour abandonment delay misses the deadline; 1 hour is correct here.
...
- **Daily scoreboard** (run each morning in Shopify admin → Analytics, or ask Claude):
  - `FROM sales SHOW orders, net_sales, average_order_value SINCE 2026-08-13 UNTIL today`
  - `FROM sales SHOW net_sales, orders GROUP BY product_title ORDER BY net_sales DESC LIMIT 10 SINCE 2026-08-13`
...
Modeled per **1,000 emailable contacts**, first-ever campaign to a cold imported list, 5 event sends + 2 warm-ups, automatic discount on site:
...
**Read your real list size in Omnisend → Audience** (it wasn't exposed via API) and multiply. Against a ~$500/week baseline, a mid-case event on a few-thousand-contact list is **several normal weeks of revenue in four days** — real, but not a lottery ticket. The three levers that move the number most, in order: (1) list size → run the popup and IG hard, (2) Sunday execution → never skip E5, (3) Traveler Set attach rate → it's 2× the AOV of a single bottle even discounted.
...
**Tuesday Aug 11**
- [ ] Verify sending domain `journey@tarifeattar.com` is DKIM/SPF-verified in Omnisend (Store settings → Sender domains) — the March drafts already use it; confirm status is "verified", otherwise sends go out on a shared domain and deliverability craters
- [ ] Confirm DMARC record exists on tarifeattar.com (any policy, even `p=none`)
- [ ] Build the 4 segments (§4) · create both discounts (§2) · restock what can be restocked
- [ ] Send WARM-UP 2 at 8am · watch bounce rate · export + suppress hard bounces
- [ ] Verify Shopify↔Omnisend order events flow (test contact timeline)

**Wednesday Aug 12**
- [ ] Add tease line to WARM-UP 4, send 8am
- [ ] Load `01-the-atlas-opens.html` into Omnisend (Campaigns → new → Code your own), replace the two `[[…]]` placeholders (unsubscribe merge tag from the editor menu + business mailing address), send test to yourself, check Gmail/Outlook/mobile rendering
- [ ] Set up A/B on E1 subject (A vs B, 20%/20% sample, winner to remainder after 4h)
- [ ] Publish exit-intent popup · stage announcement bar + homepage swap for Thursday morning
- [ ] Enable abandoned-checkout automation with event copy

**Thursday Aug 13 — open**
- [ ] E1 8am · site banner live at 7:45 · IG post 1 + story
- [ ] Evening: check scoreboard + guardrails

**Fri/Sat** — E2 9am Fri, E3 9am Sat, IG posts 2–3, monitor, re-verify stock lines for Sunday
